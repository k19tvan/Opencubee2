...
def search_ocr_on_meilisearch_sync(keyword: str, limit: int = 500, video_ids: Optional[List[str]] = None) -> List[Dict[str, Any]]:
...
async def search_ocr_on_meilisearch_async(keyword: str, limit: int = 500, video_ids: Optional[List[str]] = None):
    return await asyncio.to_thread(search_ocr_on_meilisearch_sync, keyword, limit, video_ids)
...
async def embed_and_search_model(
    ...
    raw_results = await search_qdrant(embedding, config["collection"], limit=limit)
...
async def search_all_models(
    ...
    tasks = [
        embed_and_search_model(m, text=text, image_name=image_name, image_text=image_text, limit=limit)
